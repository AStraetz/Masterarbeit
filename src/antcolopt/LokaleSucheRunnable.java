...
public class LokaleSucheRunnable implements Runnable {
	...
	public LokaleSucheRunnable(int id, int anzahlJobs, Loesung loesung, int anzahlMaschinen, int[][] ausfuehrungszeiten) {
		this.id = id;
	...
	public void run() {
		for (int aktuellerSchritt = 0; aktuellerSchritt < KonstantenUndHelper.anzahlLokaleSuche; aktuellerSchritt++) {
			if (aktuellerSchritt % anzahlJobs == 0) {
				if ((bessereLoesungGefunden == false) && (aktuellerSchritt > 1)) {
					System.out.println("Loesung " + id + ": " + loesung.berechneTFT());
					return;
				}
	...
	public String toString() {
		return "Lösung " + id + ": " + this.getLoesung().getLetztesTft();
	}

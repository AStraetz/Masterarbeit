...
public class Population {
	...
	public void testeLoesungFuerSuperpopulation(Loesung loesung) {
		...
		for (Iterator<Loesung> iterator = this.loesungenInSuperPopulation.iterator(); iterator.hasNext();) {
			...
			{iterator.remove();
			anzahlLoesungen--;}
			else {if((testeAufDominierung(loesung, loesungInSP) == -1) || (loesung.getJobreihenfolge().equals(loesungInSP.getJobreihenfolge())))
			{return;}}
		}
	...
	public int testeAufDominierung(Loesung loesung1, Loesung loesung2) {
		...
		if ((tft1 < tft2) && (tardiness1 < tardiness2)) {
			return 1;
		}
		if ((tft2 < tft1) && (tardiness2 < tardiness1)) {
			return -1;
		}
		return 0;
	}

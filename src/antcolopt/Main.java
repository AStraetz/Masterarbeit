...
public class Main {
	...
	private static final int WARTEKAPAZITAETEN_ENDWERT = 5;
	private static final int WARTEKAPAZITAETEN_STARTWERT = 4;
	private static final int PROBLEMINSTANZEN_ENDWERT = 1;
	private static final int PROBLEMINSTANZEN_STARTWERT = 0;
	private static final int PROBLEMKLASSEN_ENDWERT = 1;
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int y = PROBLEMKLASSEN_STARTWERT; y < PROBLEMKLASSEN_ENDWERT; y++) {
				...
				Reader reader = new Reader(Problem.anzahlJobsArray[y],Problem.anzahlMaschinenArray[y]);
				reader.ladeProbleminstanzen(dateiName);
				reader.ladeBestwerteTFT(Problem.dateiBewerteTft);
				Problem.berechneGesamtBearbeitungsZeitJobs(Problem.anzahlJobsArray[y],Problem.anzahlMaschinenArray[y],reader.getAusfuehrungszeiten());
				...
				for (int z = PROBLEMINSTANZEN_STARTWERT; z < PROBLEMINSTANZEN_ENDWERT; z++) {
					...
					//for(int f = 4; f<7;f++) {
						...
						for (int l = 0; l < MITTELUNG_ANZAHL; l++) {
							Problem.generiereTftHeristikLoesung(Problem.anzahlJobsArray[y],Problem.anzahlMaschinenArray[y],reader.getAusfuehrungszeiten()[z]);
							Population population = new Population(Problem.anzahlJobsArray[y], Problem.anzahlMaschinenArray[y], reader.getAusfuehrungszeiten()[z]);
						...
						bw2.write("LokaleSuche: " + Problem.anzahlLokaleSuche + " EliteGewicht: " + Problem.eliteUpdateGewicht + "gewichteter bester TFT bei Jobanzahl: " + Problem.anzahlJobsArray[y] + " Maschinenanzahl: "
								...
								+ (summe / MITTELUNG_ANZAHL) + " RPD:" + rpd +" "+ besterBekannterTft);
						bw2.write("\n");
						}
						}
					}

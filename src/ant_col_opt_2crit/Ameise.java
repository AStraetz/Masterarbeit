...
public class Ameise {
	...
	public Ameise(Loesung eliteloesung) {
		...
		aktuellAusgewaehlteLoesung = eliteloesung;
	...
	public void bestimmeNachbarschaft() {
		...
		for (int job : aktuellAusgewaehlteLoesung.getJobreihenfolge()) {
		...
		if (Problem.benutzeq0 == true) {
			...
			if (random < Problem.q0) {
				...
				if (aktuellePosition < (Problem.anzahlJobs - nachbarschaftsgroesse)) {
					nachbarschaft.add(aktuellAusgewaehlteLoesung.getJobreihenfolge()[aktuellePosition + nachbarschaftsgroesse]);
				}
				aktuellePosition++;
				return;
